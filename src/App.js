...
const reducer = (state, action) => {
  switch(action.type){
    ...
    case 'EDIT': {
      return state.map((elem) => elem.id === action.targetId ? {...elem, content: action.newContent} : elem)
    }
    default:
      return state;
  }
...
function App() {
  ...
  const getData = async() => {
    const res = await fetch("https://jsonplaceholder.typicode.com/comments").then((res) => {return res.json()});
